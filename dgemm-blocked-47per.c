...
// change all 32 to 64 by jjw
  //block size now is newlda*newldacol by jjw
  double* padA = (double*) _mm_malloc(newlda * newldacol * sizeof(double), 64);
  pad(padA, A, lda, newlda);

  double* padB = (double*) _mm_malloc(newlda * newldacol * sizeof(double), 64);
  pad(padB, B, lda, newlda);

  double* padC = (double*) _mm_malloc(newlda * newldacol * sizeof(double), 64);
